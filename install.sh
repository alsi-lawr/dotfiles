...
main() {
    ...
    gum spin --spinner dot --title "Installing zsh..." -- bash -c 'install_zsh'
    gum style --foreground 34 --border rounded --margin "0" --padding "0 1" "Installed zsh ✓"
    gum spin --spinner dot --title "Installing oh-my-zsh..." -- bash -c 'install_oh_my_zsh'
    gum style --foreground 34 --border rounded --margin "0" --padding "0 1" "Installed oh-my-zsh ✓"
    gum spin --spinner dot --title "Installing oh-my-zsh plugins..." -- bash -c 'install_ohmyzsh_plugins'
    ...
    if [[ "$RUNNING_ON_WSL" == *"wsl"* ]]; then
        gum spin --spinner dot --title "Removing windows paths, please restart WSL..." -- bash -c 'install_on_wsl '
        gum style --foreground 34 --border rounded --margin "0" --padding "0 1" "Removed windows paths ✓"
        gum spin --spinner dot --title "Adding the wincopy alias to copy file to windows clipboard..." -- bash -c 'add_wincopy'
        gum style --foreground 34 --border rounded --margin "0" --padding "0 1" "Added wincopy ✓"
    fi
    gum spin --spinner dot --title "Adding custom aliases..." -- bash -c 'add_custom_aliases'
    ...
    if [[ "$INSTALL_NVIM" == *"Yes"* ]]; then
        gum spin --spinner dot --title "Installing neovim..." -- bash -c 'install_neovim'
